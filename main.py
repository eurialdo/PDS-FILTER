...
OUTPUT_FOLDER = 'out/' # Saida dos arquivos de audio
...
class Recorder:
  ...
  def __do_record(self, chunk, channels, files_prefix, aformat=pyaudio.paInt16):
    ...
    file_output = OUTPUT_FOLDER + files_prefix + "saidadeaudio.wav"
...
class KaiserFilter:
  ...
  def add_noise_and_filter(self, x, noise, play_sounds, files_prefix):
    ...
    # Passagem de paramnetro para salvar o audio com ruido
    output_with_noise = ''.join([OUTPUT_FOLDER, files_prefix, 'Sinalcomruido.wav'])
    wavfile.write(output_with_noise, self.sample_rate, with_noise, normalized=True)

    # Substituindo sinal de X
    x = with_noise

    #-------------- Criando o filtro FIR em x -------------

    # taxa de Nyquist do sinal.
    nyq_rate = self.sample_rate / 2.0

    # A largura desejada da transição de passagem a parada em relação à taxa de Nyquist.
    width = (self.cutoff_hz_2 - self.cutoff_hz_1) / nyq_rate

    # Calcule a ordem e o parâmetro Kaiser para o filtro FIR.
    N, beta = kaiserord(self.ripple_db, width)
    N |= 1

    # Use firwin com uma janela Kaiser para criar um filtro FIR.
    ...
    # o sinal filtrado, deslocado para compensar o atraso de fase.
    figure()
    ...
    # salva audio filtrado
    output_filtered = "".join([OUTPUT_FOLDER, files_prefix, 'sinalfiltrado.wav'])
...
def main():
  ...
  parser.add_argument('--duration', '-d', default=4, type=int, help='Recording duration in seconds')
  parser.add_argument('--rate', '-r', default=44100, type=int, help='Taxa de amostragem de audio')
  parser.add_argument('--window', '-w', choices=['kaiser'], default='kaiser', help='Tipo de janela de filtro')
  parser.add_argument('--cutoffhz1', '-wc1', default=1900, type=int, help='1º Frequencia de corte')
  parser.add_argument('--cutoffhz2', '-wc2', default=2100, type=int, help='2° frequencia decorte')
  parser.add_argument('--ripple_db', '-rd', default=60, type=int, help='A atenuação desejada na banda de parada, em dB')
  parser.add_argument('--noise_1', '-n1', default=1950, type=int, help='frequencia minima de ruido')
  parser.add_argument('--noise_2', '-n2', default=2050, type=int, help='frequencia maxima de ruido')
  parser.add_argument('--play', '-p', default=True, type=bool, help='Play do audio sem ruido')
